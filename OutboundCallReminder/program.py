...
class Program():
  ...
  async def program(self):
    ...
    try:
      if (ngrok_url and len(ngrok_url)):
        ...
        Logger.log_message(Logger.INFORMATION,"Failed to start Ngrok service")
      
    except Exception as ex:
      Logger.log_message(Logger.ERROR,"Failed to start Ngrok service --> "+str(ex))
    
    Logger.log_message(Logger.INFORMATION, "Press 'Ctrl + C' to exit the sample")
  ...
  def start_ngrok_service(self):
    try:
      ...
      if (not(len(ngrokPath))):
        Logger.log_message(Logger.INFORMATION, "Ngrok path not provided")
        return None
      ...
      Logger.log_message(Logger.INFORMATION,"Ngrok Started with url -- > " + ngrok_url)
      return ngrok_url

    except Exception as ex:
      Logger.log_message(Logger.INFORMATION,"Ngrok service got failed -- > " + str(ex))
      return None


  async def run_sample(self, app_base_url):
    call_configuration = self.initiate_configuration(app_base_url)
    outbound_call_pairs = self.configuration_manager.get_app_settings("DestinationIdentities")
    ...
    try:
      if (outbound_call_pairs and len(outbound_call_pairs) ):
        ...
        for identity in identities:
          pair = identity.split(",")
          task = asyncio.ensure_future(OutboundCallReminder(call_configuration).report(pair[0].strip(), pair[1].strip())) 
          tasks.append(task)

        _ = await asyncio.gather(*tasks)

    except Exception as ex:
      Logger.log_message(Logger.ERROR, "Failed to initiate the outbound call Exception -- > " + str(ex))
        
    self.delete_user(call_configuration.connection_string, call_configuration.source_identity)
  ...
  def generate_custom_audio_message(self):
    ...
    region = configuration_manager.get_app_settings("CognitiveServiceRegion")
    custom_message = configuration_manager.get_app_settings("CustomMessage")
    ...
    try:
      if (key and len(key) and region and len(region) and custom_message and len(custom_message)):
        ...
        config.set_speech_synthesis_output_format(SpeechSynthesisOutputFormat["Riff24Khz16BitMonoPcm"])
      ...
      Logger.log_message(Logger.ERROR,"Exception while generating text to speech, falling back to sample audio. Exception -- > " + str(ex))
      return "sample-message.wav"
