# Azure VM Python Deployment Steps
...
## Download Python installer to your default Downloads folder (e.g., `C:\Users\<your-username>\Downloads`)
- Use PowerShell to download the installer (replace `<your-username>` as appropriate):
  ...
  try {
    Invoke-WebRequest -Uri $PythonUrl -OutFile "$Downloads\$PythonInstaller" -ErrorAction Stop
    Write-Host "Python download complete."
  } catch {
...
## Install Python (Silent installation with pip and Add to PATH)
```powershell
$Downloads = "$env:USERPROFILE\Downloads"
...
try {
  ...
  Start-Process -FilePath "$Downloads\$PythonInstaller" -ArgumentList $Arguments -Wait -ErrorAction Stop
...
- Open a terminal or PowerShell window and run:
  ...
  git clone https://github.com/Azure-Samples/communication-services-python-quickstarts.git gcch-test-app
  ```
...
- In IIS Manager, create a new website for your Python app:
  - Open IIS Manager and right-click **Sites** in the left panel, then select **Add Website...**
  - Enter a **Site name** (e.g., `PythonAppSite`).
  - Set the **Physical path** to the folder where your Python app is located (e.g., `C:\inetpub\wwwroot\gcch-test-app`).
  ...
  - Your new IIS site should now point to your Python app folder and be ready for further configuration.
...
- Create a URL Rewrite inbound rule in IIS to forward all HTTPS requests to your Python app at `http://localhost:8080`:
  ...
  - Set the rule name, e.g., `ReverseProxyToPython`.
  ...
  - This will forward all HTTPS traffic received by IIS to your Python app running on port 8080.
...
- Check if Python is properly installed:
  ...
  ```
- Check IIS mappings
- In Python app root folder, verify dependencies are installed:
  ```shell
  pip list
  ```
- Execute:
